...
(c%16).toString(16);})+'"';}
return'"'+this+'"';};function populateGlobals(){if(option.adsafe){globals.combine({ADSAFE:true});}else{if(option.rhino){globals.combine(rhino);}
...
character=0;s=lines[line];at=s.search(cx);if(at>=0){warningAt("Unsafe character.",line,at);}
return true;}
...
t=object(t);if(type==='(string)'){if(jx.test(value)){warningAt("Script URL.",line,from);}}else if(type==='(identifier)'){if(option.nomen&&value.charAt(0)==='_'){warningAt("Unexpected '_' in '{a}'.",line,from,value);}else if(option.adsafe&&(adsafe[value]===true||value.slice(-2)==='__')){warning("ADsafe restricted word '{a}'.",{line:line,from:character},value);}}
...
return it('(number)',t);}
switch(t){case'"':case"'":return string(t);case'//':if(src||xmode){warningAt("Unexpected comment.",line,character);}
if(option.adsafe&&ax.test(s)){warningAt("ADsafe comment violation.",line,character);}
s='';break;case'/*':if(src||xmode){warningAt("Unexpected comment.",line,character);}
if(option.adsafe&&ax.test(s)){warningAt("ADsafe comment violation.",line,character);}
...
s=s.substr(i+2);break;case'/*extern':case'/*global':case'/*members':case'/*member':case'/*jslint':case'*/':return{value:t,type:'special',line:line,character:character,from:from};case'':break;case'/':if(prereg){depth=0;captures=0;l=0;for(;;){b=true;c=s.charAt(l);l+=1;switch(c){case'':errorAt("Unclosed regular expression.",line,from);return;case'/':if(depth>0){warningAt("Unescaped '{a}'.",line,from+l,'/');}
...
function doOption(){var b,obj,filter,t,v;switch(nexttoken.value){case'*/':error("Unbegun comment.");break;case'/*extern':case'/*global':if(option.adsafe){warning("ADsafe restriction.");}
obj=globals;break;case'/*members':case'/*member':if(!membersOnly){membersOnly={};}
...
advance();if(option.adsafe&&token.value==='ADSAFE'){if(nexttoken.id!=='.'||!(peek(0).identifier)||peek(1).id!=='('){warning('ADsafe violation.',token);}}
if(initial){anonname='anonymous';verb=token.value;}
...
function assignop(s,f){symbol(s,20).exps=true;return infix(s,function(left){var l;nonadjacent(prevtoken,token);nonadjacent(token,nexttoken);if(adsafe){l=left;do{if(l.value==='ADSAFE'){warning('ADsafe violation.',l);}
l=l.left;}while(l);}
if(left){if(left.id==='.'||left.id==='['||(left.identifier&&!left.reserved)){parse(19);return left;}
...
var itself=function(s,o){if(o){if(o.adsafe){o.browser=false;o.debug=false;o.eqeqeq=true;o.evil=false;o.forin=false;o.on=false;o.rhino=false;o.undef=true;o.widget=false;}
option=o;}else{option={};}
globals=option.adsafe?{}:object(standard);JSLINT.errors=[];global=object(globals);scope=global;funct=true;functions=[];xmode=false;xtype='';stack=null;member={};membersOnly=null;implied={};inblock=false;lookahead=[];indent=0;jsonmode=false;warnings=0;lex.init(s);prereg=true;prevtoken=token=nexttoken=syntax['(begin)'];populateGlobals();try{advance();if(nexttoken.value.charAt(0)==='<'){xml();}else if(nexttoken.id==='{'||nexttoken.id==='['){option.laxbreak=true;jsonmode=true;jsonValue();}else{statements();}
